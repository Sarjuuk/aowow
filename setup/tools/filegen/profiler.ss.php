...
CLISetup::registerSetup("build", new class extends SetupScript
{
    ...
    private function titles(): void
    {
        ...
        foreach ($titlez->iterate() as $id => $tEntry)
            if (!$tEntry->getRawSource(SRC_QUEST) && !$tEntry->getRawSource(SRC_ACHIEVEMENT))
    ...
    private function recipes() : void
    {
        ...
        foreach ($skills as $s)
        {
            ...
            foreach ($recipez->iterate() as $id => $rEntry)
            {
                ...
                {
                    $icon = DB::Aowow()->selectCell('SELECT ic.name FROM ::icons ic JOIN ::items i ON i.iconId = ic.id WHERE i.id = %i', $rEntry->effectCreateItemId[$idx]);
