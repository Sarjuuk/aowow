...
function creature(array $ids = [])
{
    $baseQuery = '
        SELECT
            ...
            ct.name, ctl2.`Name`  AS n2, ctl3.`Name`  AS n3, ctl4.`Name`  AS n4, ctl6.`Name`  AS n6, ctl8.`Name`  AS n8,
            subname, ctl2.`Title` AS t2, ctl3.`Title` AS t3, ctl4.`Title` AS t4, ctl6.`Title` AS t6, ctl8.`Title` AS t8,
            minLevel, maxLevel,
            exp,
            faction,
            npcflag,
