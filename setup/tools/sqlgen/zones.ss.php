    ...
    public function generate() : bool
    {
        ...
        $baseData = DB::Aowow()->selectAssoc(
           'SELECT    a.`id`,
                      ...
                      IFNULL(wmt.`targetMapId`, IF(m.`areaType` = 1, 2, IF(m.`areaType` = 2, 3, IF(m.`areaType` = 4, 9, IF(m.`isBG` = 1, 6, IF(m.`id` = 571, 10, IF(m.`id` = 530, 8, IF(m.`id` < 2, m.`id`, 4)))))))) AS "category",
                      a.`flags`,
        ...
        // attach DeathknightStart dungeon to Eastern Kingdoms
        DB::Aowow()->query('UPDATE ::zones SET `category` = 0 WHERE `parentArea` = 4298 OR `id` = 4298');
