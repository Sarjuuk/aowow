        ...
        */
        ...
        $effects = DB::Aowow()->selectAssoc(
           'SELECT s2.id AS ARRAY_KEY,
            ...
            JOIN   ::spell s2 ON s2.effect1MiscValue = gp.id AND s2.effect1Id = 74
        ...
        foreach ($effects as $applyId => $glyphEffect)
        {
            ...
            if (!$icons)
            {
                ...
                if (is_int($search))
                    ...
                    $where[] = ['s.`SpellFamilyId` = %i AND s.`name_loc0` LIKE %~like~', $fam, $search];
            ...
            while (empty($icons) && $i < 3)
            {
                ...
                $where = "s.SpellFamilyId = %i AND (s.SpellFamilyFlags1 & %i OR s.SpellFamilyFlags2 & %i OR s.SpellFamilyFlags3 & %i)";
