...
CLISetup::registerSetup("sql", new class extends SetupScript
{
    ...
    public function generate() : bool
    {
        $baseQuery =
           'SELECT    go.entry,
                      `type`,
                      IF(`type` = 2,                                       -2,     -- cat: questgiver > quests
                          IF(`type` = 3 AND IFNULL(gqi.`ItemId`, 0) <> 0,  -2,     -- cat: contains any questitem > quests
                          IF(`type` = 8 AND `data0` IN (1, 2, 3, 4, 1552), -6,     -- cat: spellfocus subgroup > tools
                          IF(`type` IN (3, 6, 9, 19, 25),                  `type`, -- cat: is of type "regular chest, trap, book, mailbox, fish swarm" > keep type as cat
                                                                           0       -- cat: default - none
                      )))),
                      ...
                      IF(`type` = 29, CONCAT_WS(" ", `data14`, `data15`, `data16`, `data17`, `data0`),                                     -- miscInfo: capturePoint
                          IF(`type` = 3, CONCAT_WS(" ", `data4`, `data5`, `data2`),                                                        -- miscInfo: loot (chest)
                              IF(`type` = 25, CONCAT_WS(" ", `data2`, `data3`, 0),                                                         -- miscInfo: loot (fishing)
            ...
            LEFT JOIN gameobject_questitem gqi        ON go.`entry` =  gqi.`GameObjectEntry`
            GROUP BY  go.`entry`
            LIMIT     %i, %i';
        ...
        CLI::write('[objects] - updating categories..');

        // update chests typeCat and reqSkill depending on locks
        DB::Aowow()->qry(
           'UPDATE ::objects o
            JOIN   dbc_lock l ON l.`id` = o.`lockId`
            SET    `typeCat`  = IF(`type` = 3 AND (l.`properties1` = 1 OR l.`properties2` = 1), -5,  -- footlocker
                                IF(`type` = 3 AND (l.`properties1` = 2), -3,                         -- herb
                                IF(`type` = 3 AND (l.`properties1` = 3), -4, o.`typeCat`))),         -- ore
        ...
        // update uncategorized objects as interactive if they are locked
        DB::Aowow()->qry('UPDATE ::objects SET `typeCat` = -9 WHERE `typeCat` = 0 AND `lockId` > 0');
