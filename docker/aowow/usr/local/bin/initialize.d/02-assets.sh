...
download_prepared_assets() {
  ...
  curl -sL "$SETUP_MPQ_DATA_INTERFACE_URL" -o '/var/www/html/setup/mpqdata/interface/framexml/globalstrings.lua'

  # ... there is no need to save full archive and extract all contents, try to extract only required data on fly:
  curl -sL "$SETUP_EXTRACTED_DATA_ARCHIVE_URL" | bsdtar -xzf - --strip-components=1 -C '/var/www/html/setup/mpqdata/enUS/DBFilesClient' 'dbc/*'
}
