...
trait TrCommunityHelper
{
    private function handleCaption(?string $caption) : string
    {
        ...
        $caption = preg_replace('/\s{2,}/', ' ', trim(urldecode($this->_post['alt'])));
...
{
    protected  string $contentType = MIME_TYPE_JSON;
    ...
    protected function display() : void
    {
        ...
        if (Cfg::get('DEBUG') && User::isInGroup(U_GROUP_STAFF) && $this->result instanceof Tooltip)
        {
            ...
            if ($this instanceof ICache && self::$cacheStats)
            {
                [$mode, $set, $lifetime] = self::$cacheStats;
