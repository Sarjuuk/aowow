...
trait TrDetailPage
{
     ...
     * * `CreatureEntry` `CreatureContainer`
     * * `GameobjectEntry` `GameobjectContainer`
     * * `SoundEntry` `SoundContainer`
     * * `AreatriggerEntry` `AreatriggerContainer`
    ...
    private static function createFullSpawns(DBTypeEntry|DBTypeContainer $set, bool $skipWPs = false, bool $skipAdmin = false, bool $hasLabel = false, bool $hasLink = false) : array
    {
        ...
        if (!$skipWPs && $spawns && count($spawns) < 6 && $set::$dbType == Type::NPC)
            $wPoints = DB::Aowow()->selectAssoc('SELECT CONCAT(cw.`creatureOrPath`, "-", cw.`floor`) AS ARRAY_KEY, cw.point AS ARRAY_KEY2, cw.* FROM ::creature_waypoints cw WHERE cw.`creatureOrPath` IN %in AND cw.`floor` IN %in', array_column($spawns, 'creatureOrPath'), array_column($spawns, 'floor'));

        foreach ($spawns as $s)
        {
            $isAccessory = $s['guid'] < 0 && $s['type'] == Type::NPC;

            // waypoint_data.point may inconsistently start with 0 or 1 .. insert_rageface_here FFFFUUUUUUU!!!!!!!!!
            $wpForFloor = array_values($wPoints[$s['creatureOrPath'].'-'.$s['floor']] ?? []);
            foreach ($wpForFloor as $i => $p)
            {
                ...
                // connective line
                if ($i > 0 && $wpForFloor[$i - 1]['areaId'] == $p['areaId'])
                    $opts['lines'] = [[$wpForFloor[$i - 1]['posX'], $wpForFloor[$i - 1]['posY']]];
...
trait TrListPage
{
    ...
    public ?DBTypeFilter $filter          = null;
...
class TemplateResponse extends BaseResponse
{
 ...
 // protected static array $cacheStats = [];
    ...
    public function extendGlobalData(array $data, ?array $extra = null) : void
    {
        ...
        if ($extra)
        {
            [$type, $typeId, $extraData] = $extra;
            $namedExtra = [];
            foreach ($extraData as $k => $v)
                $namedExtra[$k.'_'.Lang::getLocale()->json()] = $v;

            $this->jsGlobals[$type][2] = [$typeId => $namedExtra];
        }
    }
    ...
    private function applyGlobals() : void
    {
        foreach ($this->jsgBuffer as $type => $ids)
        {
            ...
            $this->extendGlobalData($obj->getJSGlobals());
