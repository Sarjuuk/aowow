...
trait TrCache
{
    ...
    protected static array $cacheStats = [];                // load info for page footer
    ...
    public function saveCache(string|object $toCache) : void
    {
        ...
        if (Cfg::get('CACHE_MODE') & CACHE_MODE_MEMCACHED)
        {
            ...
            if ($this->skipCache == CACHE_MODE_MEMCACHED)
            {
                ...
                $search = substr($this->formatCacheKey()[2], 0, 19);
...
trait TrProfilerDetail
{
    ...
    private function handleIncompleteData(int $type, int $guid) : void
    {
        ...
        if ($newId = Profiler::scheduleResync($type, $this->realmId, $guid))
        {
            ...
            $this->inputbox = ['inputbox-status', ['head' => Lang::profiler('firstUseTitle', [$this->subjectName, $this->realm])]];
...
abstract class BaseResponse
{
    ...
    protected static function checkUsername(string $val) : string
    {
        if (User::isValidName(Util::lower(trim(urldecode($val)))))
            return $val;
