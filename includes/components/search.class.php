...
class Search
{
    ...
    public function __construct(private string $query, private int $moduleMask = -1, private array $extraCnd = [], private array $queryOpts = [], private int $maxResults = self::DEFAULT_MAX_RESULTS)
    {
        $this->tokenizeQuery();
    ...
    private function tokenizeQuery() : void
    {
        ...
        {
            if ([$like, $fulltext, $ex] = DBTypeFilter::transformToken($raw, $allowShort))
            {
    ...
    public function canPerform() : bool
    {
        ...
        return (($this->included || $this->queryOpts)) && $this->moduleMask;
    }
    ...
    private function _searchItemset(array &$shared) : ?array// 5 Itemsets $moduleMask & 0x0000020
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            Util::mergeJsGlobals($this->jsgStore, $sets->getJSGlobals());
    ...
    private function _searchItem(array &$shared) : ?array   // 6 Items $moduleMask & 0x0000040
    {
        ...
        if ($this->moduleMask & self::TYPE_JSON)
        {
            ...
            else
            {
                ...
                if ($this->queryOpts)
                    $miscData['queryOpts'] = $this->queryOpts;
                if ($this->extraCnd)
    ...
    private function _searchAbility() : ?array              // 7 Abilities (Player + Pet) $moduleMask & 0x0000080
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            Util::mergeJsGlobals($this->jsgStore, $abilities->getJSGlobals(GLOBALINFO_RELATED));

            $vis = ['level', 'schools'];
            if ($abilities->hasSetFields('reagent'))
                $vis[] = 'reagents';

            if ($abilities->hasSetFields('reqClassMask'))
    ...
    private function _searchTalent() : ?array               // 8 Talents (Player + Pet) $moduleMask & 0x0000100
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            ...
            if ($talents->hasSetFields('reagent'))
                $vis[] = 'reagents';
    ...
    private function _searchProfession() : ?array           // 11 Professions (Primary + Secondary) $moduleMask & 0x0000800
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            Util::mergeJsGlobals($this->jsgStore, $prof->getJSGlobals(GLOBALINFO_RELATED));
    ...
    private function _searchAchievement() : ?array          // 16 Achievements $moduleMask & 0x0010000
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            Util::mergeJsGlobals($this->jsgStore, $acvs->getJSGlobals(GLOBALINFO_REWARDS));
    ...
    private function _searchStatistic() : ?array            // 17 Statistics $moduleMask & 0x0020000
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            Util::mergeJsGlobals($this->jsgStore, $stats->getJSGlobals());
    ...
    private function _searchSpell() : ?array                // 24 Spells (Misc + GM + triggered abilities) $moduleMask & 0x1000000
    {
        ...
        if ($this->moduleMask & self::TYPE_REGULAR)
        {
            Util::mergeJsGlobals($this->jsgStore, $misc->getJSGlobals(GLOBALINFO_RELATED));
