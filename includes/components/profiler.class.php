...
class Profiler
{
    ...
    */
    ...
    public static function getCharFromRealm(int $realmId, int $charGuid) : int
    {
        ...
        {
            DB::Aowow()->qry('UPDATE ::profiler_profiles SET `lastupdated` = %i WHERE `id` = %i', time(), $profileId);
        ...
        {
            if ($ri = DB::Aowow()->selectCell('SELECT MAX(`renameItr`) FROM ::profiler_profiles WHERE `realm` = %i AND `realmGUID` = %i', $realmId, $charGuid))
                $data['renameItr'] = $ri;
            else if ($ri = DB::Aowow()->selectCell('SELECT MAX(`renameItr`) FROM ::profiler_profiles WHERE `realm` = %i AND `custom` = 0 AND `name` = %s', $realmId, $char['name']))
                $data['renameItr'] = ++$ri;
            else
                $data['renameItr'] = 1;
        }
