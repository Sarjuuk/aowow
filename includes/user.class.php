...
class User
{
    ...
    public static function init()
    {
        ...
        else if ($session['userId'] != $userData['id'])        // what in the name of fuck..?
        {
            ...
            trigger_error(__METHOD__.' - tried to resume session "'.session_id().'" of user #'.$_SESSION['user'].' linked to session data for user #'.$session['userId'].' Kicked both!', E_USER_WARNING);
            self::destroy();
            return false;
        }
    ...
    private static function authExtern(string $nameOrEmail, #[\SensitiveParameter] string $password, int &$userId) : int
    {
        ...
        {
            trigger_error(__METHOD__.' - AUTH_MODE_EXTERNAL is selected but config/extAuth.php does not exist!', E_USER_WARNING);
            return AUTH_INTERNAL_ERR;
        }
        ...
        {
            trigger_error(__METHOD__.' - AUTH_MODE_EXTERNAL is selected but function extAuth() is not defined!', E_USER_WARNING);
            return AUTH_INTERNAL_ERR;
        }
