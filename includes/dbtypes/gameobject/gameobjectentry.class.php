...
class GameobjectEntry extends DBTypeEntry implements ISource, ITooltip
{
 ...
 // public readonly  bool      $endsQuests;                 // only defined for filtering
    ...
    public function applyInitData(array $initData, array $opts) : void
    {
        ...
        $this->reqSkill     = $initData['reqSkill'];
        $this->pageTextId   = $initData['pageTextId'];
        $this->linkedTrap   = $initData['linkedTrap'];
        $this->reqQuest     = $initData['reqQuest'];
        $this->spellFocusId = $initData['spellFocusId'];
    ...
    {
        $data = DB::Aowow()->selectAssoc('SELECT `id` AS ARRAY_KEY, `name_loc0`, `name_loc2`, `name_loc3`, `name_loc4`, `name_loc6`, `name_loc8` FROM %n WHERE `id` IN %in', static::$dataTable, $ids) ?: [];
