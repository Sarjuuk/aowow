...
class ProfileFilter extends DBTypeFilter
{
    ...
    */
    ...
    protected function cbCompletedAcv(int $cr, int $crs, string $crv) : ?array
    {
        ...
        if ($this->useLocalList)
        {
            $this->queryOpts[$k] = ['j' => [sprintf('::profiler_completion_achievements %1$s ON `%1$s`.`id` = p.`id` AND `%1$s`.`achievementId` = %2$d', $k, $crv), true]];
            return [$k.'.achievementId', null, '!'];
        }
        else
        {
            $this->queryOpts[$k] = ['j' => [sprintf('character_achievement %1$s ON `%1$s`.`guid` = c.`guid` AND `%1$s`.`achievement` = %2$d', $k, $crv), true]];
    ...
    protected function cbEquippedItemProp(int $cr, int $crs, string $crv, int $type, string $field) : ?array
    {
        ...
        $this->queryOpts[$k] = ['j' => [sprintf('::profiler_items %1$s ON `%1$s`.`id` = p.`id` AND `%1$s`.`%3$s` = %2$d', $k, $crv, $field), true]];
        return [$k.'.'.$field, null, '!'];
    }
