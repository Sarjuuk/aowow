...
class ItemFilter extends DBTypeFilter
{
    ...
    public function createConditionsForWeights() : array
    {
        ...
        if ($select)
        {
            $this->queryOpts['is']['s'][] = ', IF(`is`.`typeId` IS NULL, 0, ('.implode(' + ', $select).') / '.$wtSum.') AS "weightScore"';
            $this->queryOpts['is']['o'][] = 'weightScore DESC';
            $this->queryOpts['i']['o'][]  = null;           // remove default ordering
        }
        else
            $this->queryOpts['is']['s'][] = ', 0 AS "weightScore"'; // prevent errors
    ...
    protected function cbHasRandEnchant(int $cr, int $crs, string $crv) : ?array
    {
        ...
        if (count(array_unique(array_column($randIds, 'name_loc'. Lang::getLocale()->value))) == 1 && reset($randIds)['name_loc'.Lang::getLocale()->value])
            $this->queryOpts['relEnchant'] = $tplIds;
        // try EN fallback
    ...
    protected function cbCooldown(int $cr, int $crs, string $crv) : ?array
    {
        ...
        $this->fiExtraCols[] = $cr;
        $this->queryOpts['is']['s'][] = ', GREATEST(`spellCooldown1`, `spellCooldown2`, `spellCooldown3`, `spellCooldown4`, `spellCooldown5`) AS "cooldown"';
