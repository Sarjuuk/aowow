...
class CreatureFilter extends DBTypeFilter
{
    ...
    protected function cbHealthMana(int $cr, int $crs, string $crv, string $minField, string $maxField) : ?array
    {
        ...
        switch ($crs)
        {
            case '=':                               // min > max is totally possible
                $this->queryOpts['ct']['h'][] = $minField.' = '.$maxField.' AND '.$minField.' = '.$crv;
                break;
            case '>':
            case '>=':
            case '<':
            case '<=':
                $this->queryOpts['ct']['h'][] = 'IF('.$minField.' > '.$maxField.', '.$maxField.', '.$minField.') '.$crs.' '.$crv;
                break;
        }
