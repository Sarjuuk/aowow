...
abstract class Util
{
    ...
    */
    /**
     * get zone ids for locations-column in listview, sorted by spawn points DESC
     *
     * @param null|GameobjectContainer|CreatureContainer $set
     * @param null|GameobjectEntry|CreatureEntry $entry
     *
     * @return array<int, int[]> typeId => [zoneId1, zoneId2, ..]
     */
    public static function createZoneSpawns(null|GameobjectContainer|CreatureContainer $set = null, null|GameobjectEntry|CreatureEntry $entry = null) : array
    {
        if (!$set && !$entry)
            return [];
        ...
        $result = DB::Aowow()->selectCol(
           'SELECT `typeId` AS ARRAY_KEY, GROUP_CONCAT(`areaId` ORDER BY `n` DESC)
            ...
            GROUP BY `typeId`',
            $set ? $set::$dbType       : $entry::$dbType,
            $set ? $set->getfoundIDs() : [$entry->id]
        ) ?: [];
