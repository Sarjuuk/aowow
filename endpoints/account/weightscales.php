...
class AccountWeightscalesResponse extends TextResponse
{
    ...
    private function storeScaleData(int $scaleId) : void
    {
        DB::Aowow()->query('DELETE FROM ?_account_weightscale_data WHERE `id` = ?d', $scaleId);

        foreach ($this->_post['scale'] as [$k, $v])
            if (in_array($k, Util::$weightScales))          // $v is known to be a positive int due to regex check
                DB::Aowow()->query('INSERT INTO ?_account_weightscale_data VALUES (?d, ?, ?d)', $scaleId, $k, $v);
    }
    ...
    private static function checkScale(string $val) : array
    {
    ...
    private static function checkName(string $val) : string
    {
        return filter_var(trim(urldecode($val)), FILTER_SANITIZE_SPECIAL_CHARS, FILTER_FLAG_STRIP_LOW);
    }
}
