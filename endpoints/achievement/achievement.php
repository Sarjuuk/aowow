...
class AchievementBaseResponse extends TemplateResponse implements ICache
{
    ...
    protected function generate() : void
    {
        ...
        foreach ($this->subject->getCriteria() as $crt)
        {
            ...
            switch ($crt['type'])
            {
                ...
                case ACHIEVEMENT_CRITERIA_TYPE_KILLED_BY_CREATURE:
                    $crtIcon = new IconElement(Type::NPC, $obj, $crtName ?: CreatureList::getName($obj), size: IconElement::SIZE_SMALL, element: 'iconlist-icon', extraText: $killSuffix);
                    break;
                // link to area (by map)
        ...
        if ($this->createMail())
            $this->result->addScript(SC_CSS_FILE, 'css/Book.css');
