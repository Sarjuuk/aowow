...
class GuideBaseResponse extends TemplateResponse implements ICache
{
    ...
    protected function generateMetadata(bool $useArticle = true) : void
    {
        ...
        $keywords = array(
            ...
            Lang::guide('editor', 'category').Lang::main('colon').Lang::guide('category', $this->subject->category)
        );

        if ($this->subject->category == 1)
        {
            if ($cl = $this->subject->classId)
                $keywords[] = Lang::game('cl', $cl);
            if ($cl && ($s = $this->subject->specId) >=0)
                $keywords[] = Lang::game('classSpecs', $cl, $s).' '.Lang::game('cl', $cl);
        }

        array_unshift($this->metaTags, ['name' => 'keywords', 'content' => $keywords]);

        $this->buildBasicMetadata($this->subject->description, useArticle: false);
