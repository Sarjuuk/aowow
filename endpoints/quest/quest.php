...
class QuestBaseResponse extends TemplateResponse implements ICache
{
    ...
    protected function generate() : void
    {
        ...
        if ($mapNPCs)
        {
            if (!($npcs = new CreatureContainer(array(['id', array_column($mapNPCs, 0)])))->error)
            {
                ...
                $addObjectiveSpawns($spawns, function (array $npcData, int $npcId) use ($mapNPCs, $itemObjectives, &$hasStartEnd)
                {
                    ...
                    if ($startItemId)
                        $npcData['item'] = ItemEntry::getName($startItemId);
        ...
        if ($mapGOs)
        {
            if (!($gos = new GameobjectContainer(array(['id', array_column($mapGOs, 0)])))->error)
            {
                ...
                $addObjectiveSpawns($spawns, function (array $goData, int $goId) use ($mapGOs, $itemObjectives, &$hasStartEnd)
                {
                    ...
                    if ($startItemId)
                        $goData['item'] = ItemEntry::getName($startItemId);
