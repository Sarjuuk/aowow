...
class UserBaseResponse extends TemplateResponse
{
    ...
    protected function generate() : void
    {
        ...
        if (User::isInGroup(U_GROUP_STAFF))
        {
            ...
            $infobox[] = Lang::account('email') . $this->user['email'];
        }
        ...
        $infobox[] = Lang::user('userGroups')  . ($groups ? implode(', ', $groups) : Lang::account('groups', -1));
        ...
        if ($this->user['avatar'])
        {
            ...
            {
                1       => $this->user['wowicon'],
                2       => DB::Aowow()->selectCell('SELECT `id` FROM ?_account_avatars WHERE `current` = 1 AND `userId` = ?d', User::$id),
                default => ''
            };
        ...
        if ($this->user['description'])                                 // seen CLASS_STAFF, but wouldn't dare.. filtered for restricted tags before sent?
            $this->description = new Markup($this->user['description'], ['allow' => User::isPremium() ? Markup::CLASS_PREMIUM : Markup::CLASS_USER], 'description-generic');
        ...
        $this->lvTabs = new Tabs(['parent' => "\$\$WH.ge('tabs-generic')"], 'relTabs', true);
