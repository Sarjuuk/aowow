...
class ZoneBaseResponse extends TemplateResponse implements ICache
{
    ...
    protected function generate() : void
    {
        ...
        if (in_array($this->subject->category, [MAP_TYPE_DUNGEON, MAP_TYPE_RAID]))
        {
            ...
            foreach ($items->iterate() as $id => $itemEntry)
            {
                ...
                if ($map & SRC_FLAG_RAID_DROP)
                    ...
                    $mode = ($src & 0x1 ? 0x2 : 0) | ($src & 0x2 ? 0x1 : 0);
        ...
        if ($cSpawns && !$creatureSpawns->error)
        {
            ...
            $this->extendGlobalData($creatureSpawns->getJSGlobals());
        ...
        if ($oSpawns && !$objectSpawns->error)
        {
            ...
            $this->extendGlobalData($objectSpawns->getJSGlobals());
        ...
        if ($questStartItem)
        {
            ...
            if (!$qsiList->error)
            {
                ...
                $this->extendGlobalData($qsiList->getJSGlobals());
            }
        }
        ...
        if ($rewardsLV)
        {
            ...
            if (!$rewards->error)
            {
                ...
                $this->extendGlobalData($rewards->getJSGlobals());
            }
        }
        ...
        if (!$subZones->error)
        {
            ...
            $this->extendGlobalData($subZones->getJSGlobals());
        }
        ...
        if ($soundIds)
        {
            ...
            if (!$music->error)
            {
                ...
                $this->extendGlobalData($music->getJSGlobals());
    ...
    protected function generateMetadata(bool $useArticle = true) : void
    {
        ...
        $cat = $this->subject->category;

        $keywords = [$this->h1, Util::ucFirst(Lang::game('zone')), Lang::zone('cat', $cat), Lang::zone('territories', $this->subject->faction)];

        $n = Lang::zone('territories', $this->subject->faction).' '.Lang::game('zone');
        ...
        if ($cat == 2 || $cat == 3)
            $keywords[] = Lang::game('expansions', $this->subject->expansion);
