...
class ClassBaseResponse extends TemplateResponse implements ICache
{
    ...
    protected function generate() : void
    {
        ...
        $conditions = array(
            ...
            [
                ...
                ['s.skillLine2OrMask', $this->subject->skills]
            ],
        ...
        $skillIds  = [];
        if (!($genSpells = new SpellContainer($conditions))->error)
        {
            $this->extendGlobalData($genSpells->getJSGlobals(GLOBALINFO_RELATED));
        ...
        // tab: races
        if (!($races = new CharRaceContainer(array(['classMask', $cl->toMask(), '&'])))->error)
        ...
        if (!($trainer = new CreatureContainer($conditions))->error)
        {
        ...
        $itemOpts = array(
            'apply'     => ['skipStats' => true],               // during apply skip creating StatsContainer
            'queryOpts' => ['i' => [0 => ['src', 'ic']]]        // only join sources and icons by default
        );

        if (!($items = new ItemContainer($conditions, $itemOpts))->error)
        {
        ...
        // tab: itemsets
        if (!($sets = new ItemsetContainer(array(['classMask', $cl->toMask(), '&'])))->error)
        {
            $this->extendGlobalData($sets->getJSGlobals());
        ...
        if (!($quests = new QuestContainer($conditions))->error)
        {
        ...
        if (!($crtOf = new AchievementContainer($conditions))->error)
        {
    ...
    protected function generateMetadata(bool $useArticle = true) : void
    {
        ...
        $this->buildBasicMetadata(icon: $this->subject->icon);
