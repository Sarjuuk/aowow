	...
	tags: {
			...
			toHtml: function(attr)
			{
				...
				if(size == "tiny") {
					...
					if(attr.unnamed == undefined)
					{
						...
						var src = '';
						if(attr.name)
							src = g_staticUrl + '/images/icons/tiny/' + attr.name.toLowerCase() + '.gif';
						else if(attr.preset)
		...
		img:
		{
			...
			toHtml: function(attr)
			{
				...
				if(attr.src)
					...
					str += ' src="' + g_staticUrl + '/images/icons/' + (attr.size ? attr.size : 'large') + '/' + attr.icon + '.jpg"';
				else if(attr.blog)
				{
		...
		spell:
		{
			...
			attr:
			{
				...
				mop: { req: false, valid: /^[0-9]+$/ },
				mopname: { req: false, valid: /\S+/ },
				mopicon: { req: false, valid: /\S+/ }
			},
			validate: function(attr)
			{
				if((attr.domain || attr.site) && Markup.dbpage)
