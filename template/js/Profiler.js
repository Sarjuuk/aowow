...
/* end custom */
    ...
    function _updateInfoboxSpec() {
        ...
        spSpec.style.backgroundImage = 'url(' + g_staticUrl + '/images/icons/tiny/' + specData.icon.toLowerCase() + '.gif)';
    ...
    function _updateInfoboxActivity() {
        ...
        for (var i = _progress.length - 1; i >= 0; --i) {
            ...
            // Completion
            var sp = $WH.ce('span');
            sp.style.backgroundImage = 'url(' + g_staticUrl + '/template/images/' + (hasMeta ? 'tick.png' : (nKills ? 'cog.gif' : 'delete.gif')) + ')';
...
function ProfilerTalents(_parent) {
    ...
    function _updateBuild(build) {
        ...
        if (_mode == 'pets') {
            ...
            if (sp) {
                sp.style.backgroundImage = 'url(' + g_staticUrl + '/images/icons/tiny/' + specData.icon.toLowerCase() + '.gif)';
...
function ProfilerInventory(_parent) {
    var
        ...
        _proficiencies = {
             ...
             2: { 2: [0, 1, 4, 5, 6, 7, 8],                           4: [6, 11, 1, 2, 3, 4] },
             3: { 2: [0, 1, 2, 3, 6, 7, 8, 10, 13, 15, 16, 18],       4: [1, 2, 3]           },
             4: { 2: [2, 3, 4, 7, 13, 15, 16, 18],                    4: [1, 2]              },
             5: { 2: [4, 10, 15, 19],                                 4: [1]                 },
             6: { 2: [0, 1, 4, 5, 6, 7, 8],                           4: [11, 1, 2, 3, 4]    },
             7: { 2: [0, 1, 4, 5, 10, 13, 15],                        4: [6, 11, 1, 2, 3]    },
             8: { 2: [7, 10, 15, 19],                                 4: [1]                 },
             9: { 2: [7, 10, 15, 19],                                 4: [1]                 },
            11: { 2: [4, 5, 6, 10, 13, 15],                           4: [11, 1, 2]          }
        },
...
function ProfilerCompletion(_parent) {
        ...
        extraCols = [
            {
                ...
                compute: function (row, td) {
                    var img = $WH.ce('img');
                    img.src = (row.completed ? g_staticUrl + '/template/images/' + (!row._included() ? 'star' : 'tick') + '.png' : g_staticUrl + '/template/images/delete.gif');
    ...
    this.initialize = function (container, tabIndex, opt) {
        ...
        _imgLoading.src = g_staticUrl + '/template/images/ui/misc/progress-anim.gif';
    ...
    this.onShow = function () {
        ...
        if (_opt.onDemand && !_loading && !_loaded) {
            ...
            $WH.g_ajaxIshRequest(g_staticUrl + '?data=' + _opt.onDemand + (_opt.dataArgs ? _opt.dataArgs(_profile) : '') + '&t=' + g_dataKey + '&callback=$WowheadProfiler.loadOnDemand&' + (new Date().getTime()));
            ...
            case 0:
                ...
                if (_category !== null) {
                    if (_opt.onDemand && _opt.partial && !_loaded[_category] && !_loading) {
                        ...
                        $WH.g_ajaxIshRequest(g_staticUrl + '?data=' + _opt.onDemand + '&catg=' + _category + '&t=' + g_dataKey + '&callback=$WowheadProfiler.loadOnDemand&' + (new Date().getTime()));
                        _imgLoading.style.display = 'block';
                        _divTipQuests.style.display = 'none';
                        _loading = this;
                        _category = null;
    ...
    columns: [
        {
            ...
            compute: function (row, td, tr) {
                ...
                img.src = (row.complete == row.total ? g_staticUrl + '/template/images/tick.png' : g_staticUrl + '/template/images/delete.gif');
                img.onmouseover = function (e) {
...
Listview.templates.gallery = {
    ...
    compute: function (spell, td, i) {
        ...
        var img = $WH.ce('img');
        img.src = (spell.completed ? g_staticUrl + '/template/images/' + (!spell._included() ? 'star' : 'tick') + '.png' : g_staticUrl + '/template/images/delete.gif');
        img.onmouseover = function (e) {
